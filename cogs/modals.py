...
class RegistrationModal(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if role:  # Если роль найдена, выдать её пользователю
...
class RegistraionSelect(disnake.ui.Select):
    def __init__(self):
        options = [
            ...
            disnake.SelectOption(label="📝 Секретарь", value="секретарь", description="Выполняет поручения мэра"),
...
class RegistrationCog(commands.Cog):
    ...
    async def registration(self):
        ...
        embed.description = "Для вступления в ряды граждан нашего **города** и получения доступа ко всем **каналам**, \nвам необходимо пройти короткую **регистрацию** \n" \
                            "Ниже вы можете выбрать желаемую профессию, нажав на кнопку роли в меню выбора, ее всегда можно будет изменить" \
